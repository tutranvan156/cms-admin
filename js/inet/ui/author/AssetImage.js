...
$(function () {
  ...
  iNet.ui.author.AssetImage = function (options) {
    ...
    this.toolbar = {
      UPLOAD: $('#folder-btn-create'),
      BACK: $('#folder-btn-remove'),
      SELECT: $('#folder-btn-published')
    };
  ...
  iNet.extend(iNet.ui.author.AssetImage, iNet.ui.CMSComponent, {
    init: function () {
    ...
    load: function (type, folder) {
      var _this = this;
      this.setType(type || CMSConfig.ASSET_TYPE_IMAGE);
      AssetAPI.list({type: this.getType(), category: folder}, function (results) {
      ...
      if (this.files.length > 0) {
        ...
        uploadSingle(idx, this.files, this.submiter, {
          ...
          onProgress: function (value) {
            console.log('progress index ', idx);
            getProgressElByIndex(idx).css({width: String.format("{0}%", value)});
          },
          onComplete: function (e) {
            console.log('complete index ', idx, e);
          },
          onError: function (e) {
            console.log('error index ', idx, e);
          }
        });
      }
    }
